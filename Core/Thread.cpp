...
static void SetThreadName(const DWORD dwThreadId, const LPCSTR szName)
{
...
bool Thread::Join(const uint32_t TimeoutMs) const
{
    assert(m_Handle);
    const DWORD dwReason = WaitForSingleObject(m_Handle, TimeoutMs);
...
{
    assert(m_Handle);
    const DWORD dwReason = WaitForSingleObject(m_Handle, 0);
...
{
    const auto pThread = static_cast<Thread*>(pParam);
    pThread->Run();
    Exit(0);
}

void Thread::Exit(const uint32_t ErrorCode)
{
