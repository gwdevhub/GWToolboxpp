...
namespace OSHGui {
	namespace Drawing {
		const int Direct3D9TextureTarget::DefaultSize = 128;
		//---------------------------------------------------------------------------
		//Constructor
		//---------------------------------------------------------------------------
		Direct3D9TextureTarget::Direct3D9TextureTarget(Direct3D9Renderer &owner)
			...
			DeclareRenderSize(SizeI(DefaultSize, DefaultSize));
		}
		//---------------------------------------------------------------------------
		...
		//Getter/Setter
		//---------------------------------------------------------------------------
		void Direct3D9TextureTarget::DeclareRenderSize(const SizeI &size) {
			if (area.GetWidth() >= size.Width && area.GetHeight() >= size.Height) {
				return;
			}

			SetArea(RectangleI(area.GetLocation(), size));
			ResizeRenderTexture();
			Clear();
		}
		//---------------------------------------------------------------------------
		...
		void Direct3D9TextureTarget::InitialiseRenderTexture() {
			Drawing::SizeI size = owner.GetAdjustedSize(area.GetSize());

			owner.GetDevice()->CreateTexture(static_cast<UINT>(size.Width), static_cast<UINT>(size.Height), 
				1U, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &d3d9Texture, nullptr);
