...
namespace OSHGui {
	namespace Drawing {
		...
		uint32_t FreeTypeFont::GetTextureSize(CodepointIterator start, CodepointIterator end) const {
			uint32_t size = 32;
			uint32_t maximum = Application::Instance().GetRenderer().GetMaximumTextureSize();
			uint32_t count = 0;
		...
		void FreeTypeFont::Rasterise(uint32_t startCodepoint, uint32_t endCodepoint) const {
			...
			while (true) {
				uint32_t textureSize = GetTextureSize(start, end);
				if (textureSize == 0) {
					break;
				}

				Drawing::TexturePtr texture = Application::Instance().GetRenderer().CreateTexture(SizeI(textureSize, textureSize));
				...
				while (start != glyphMap.end()) {
					...
					if (!start->second.GetImage()) {
						if (FT_Load_Char(fontFace, start->first, FT_LOAD_RENDER | (antiAliased ? FT_LOAD_TARGET_NORMAL : FT_LOAD_TARGET_MONO)) != 0) {
							ImagePtr image = std::make_shared<Image>(texture, RectangleI(0, 0, 0, 0), PointI(0, 0));
							...
							RectangleI area(x, y, glyphWidth - GlyphPadding, glyphHeight - GlyphPadding);
							...
							ImagePtr image = std::make_shared<Image>(texture, area, offset.cast<int>());
							glyphImages.push_back(image);
							start->second.SetImage(image);
				...
				texture->LoadFromMemory(buffer.data(), SizeI(textureSize, textureSize), Texture::PixelFormat::RGBA);
		...
		void FreeTypeFont::InitialiseFontGlyph(CodepointIterator it) const {
			...
			float advance = fontFace->glyph->metrics.horiAdvance * FT_PosCoefficient;
