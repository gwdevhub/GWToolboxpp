...
void ChatCommands::CmdTP(const wchar_t *message, int argc, LPWSTR *argv) {
	...
	if (town.compare(0, 3, L"fav", 3) == 0) {
		...
		TravelWindow::Instance().Travel(GW::Constants::MapID::Urgozs_Warren, district, district_number);
	} else if (town == L"deep") {
		TravelWindow::Instance().Travel(GW::Constants::MapID::The_Deep, district, district_number);
	} else if (town == L"gtob") {
