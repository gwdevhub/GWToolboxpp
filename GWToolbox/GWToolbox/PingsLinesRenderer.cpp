...
PingsLinesRenderer::PingsLinesRenderer() : vertices(nullptr) {
	...
	GW::StoC().AddGameServerEvent<GW::Packet::StoC::P148>(
		[this](GW::Packet::StoC::P148* pak) -> bool {
		if (pak->Value_id == 20 && pak->value == 928) {
			printf("P148: id %d, caster %d, target %d, value %d\n",
				pak->Value_id, pak->caster, pak->target, pak->value);
		}
		if (pak->Value_id == 20
	...
	GW::StoC().AddGameServerEvent<GW::Packet::StoC::P216>(
		[this](GW::Packet::StoC::P216* pak) -> bool {
