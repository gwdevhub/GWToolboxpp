...
HotkeyFlagHero::HotkeyFlagHero(CSimpleIni* ini, const char* section) : TBHotkey(ini, section) {
	degree = ini ? (float)ini->GetDoubleValue(section, "degree", 0.0) : 0.0f;
	distance = ini ? (float)ini->GetDoubleValue(section, "distance", 0.0) : 0.0f;
	mapid = ini ? ini->GetLongValue(section, "mapid", 0) : 0;
	hero = ini ? ini->GetLongValue(section, "hero", 0) : 0;
}
...
void HotkeyFlagHero::Description(char* buf, int bufsz) const {
	if (hero == 0) {
...
void HotkeyFlagHero::Draw() {
	if (ImGui::InputFloat("Degree", &degree, 0.0f, 0.0f, 3)) hotkeys_changed = true;
	if (ImGui::InputFloat("Distance", &distance, 0.0f, 0.0f, 3)) hotkeys_changed = true;
	if (ImGui::InputInt("Map", (int*)&mapid, 0)) hotkeys_changed = true;
	ImGui::ShowHelp("The hotkey will only trigger in this map.\nUse 0 for any map.");
	if (ImGui::InputInt("Hero", (int*)&hero, 0)) hotkeys_changed = true;
...
void HotkeyFlagHero::Execute() {
	...
	if (hero == 0) {
		...
		if (hero > flags.size()) return;

		GW::HeroFlag& flag = flags[hero - 1];
	...
	double reference_radiant = player->rotation_angle;
	...
	double x = player->x + distance * std::cos(reference_radiant - radiant);
	double y = player->y + distance * std::sin(reference_radiant - radiant);

	GW::GamePos pos = GW::GamePos((float) x, (float) y, 0);
