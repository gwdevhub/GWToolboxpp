...
namespace SearingTest {
	int IsPresearing() {
		int mapid = (int)GW::Map::GetMapID();
		if ((mapid >= 145 && mapid <= 148) || (mapid >= 160 && mapid <= 166)) {
			return 5;
		}
		else {
			return 180;
...
void TravelWindow::Initialize() {
	...
	N_OUTPOSTS = SearingTest::IsPresearing();
}
...
void TravelWindow::Draw(IDirect3DDevice9* pDevice) {
	...
	if (ImGui::Begin(Name(), GetVisiblePtr(), GetWinFlags())) {
		...
		if (ImGui::MyCombo("Travel To...", &travelto_index, outpost_name_array_getter, nullptr, N_OUTPOSTS)) {
...
GW::Constants::MapID TravelWindow::IndexToOutpostID(int index) {
	...
	if (N_OUTPOSTS <= 5) {
		switch (index) {
		case 0: return MapID::Ascalon_City_pre_searing;
		case 1: return MapID::The_Barradin_Estate_outpost;
		case 2: return MapID::Ashford_Abbey_outpost;
		case 3: return MapID::Foibles_Fair_outpost;
		case 4: return MapID::Fort_Ranik_pre_Searing_outpost;
		default: return MapID::Ascalon_City_pre_searing;
		}
	}
	else {
		switch (index) {
...
namespace {
	bool outpost_name_array_getter(void* data, int idx, const char** out_text) {
		...
		if(N_OUTPOSTS <= 5) {
			switch (idx) {
			case 0: *out_text = "Pre Searing Ascalon";			break;
			case 1: *out_text = "Pre Searing The Barradin Estate"; break;
			case 2: *out_text = "Pre Searing Ashford Abbey";	break;
			case 3: *out_text = "Pre Searing Foibles Fair";		break;
			case 4: *out_text = "Pre Searing Fort Ranik";		break;
			default: *out_text = "";
			}
		}
		else {
