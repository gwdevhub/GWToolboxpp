...
void GWToolbox::Initialize()
{
    ...
    Resources::Instance().EnsureFileExists(Resources::GetPath(L"GWToolbox.ini"),
                                           ...
                                           [](bool success) {
                                               if (success) {
                                                   Instance().OpenSettingsFile();
                                                   Instance().LoadModuleSettings();
                                               }
                                           });
    ...
    if (GW::Map::GetInstanceType() != GW::Constants::InstanceType::Loading
        ...
        const DWORD playerNumber = GW::Agents::GetPlayerAsAgentLiving()->player_number;
...
void GWToolbox::SaveSettings()
{
    ...
    if (inifile)
        inifile->SaveFile(Resources::GetPath(L"GWToolbox.ini").c_str());
}
...
{
    static HWND gw_window_handle = nullptr;

    // === initialization ===
    if (!tb_initialized && !Instance().must_self_destruct) {
    ...
    if (tb_initialized && Instance().must_self_destruct) {
        ...
        SetWindowLongPtr(gw_window_handle, GWL_WNDPROC, static_cast<long>(OldWndProc));
