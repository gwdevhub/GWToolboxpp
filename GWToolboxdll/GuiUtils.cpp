...
void GuiUtils::LoadFonts()
{
    ...
    std::thread t([] {
        ...
        if (PathFileExistsW(Resources::GetPath(L"Font.ttf").c_str())) {
            const utf8::string f = Resources::GetPathUtf8(L"Font.ttf");
            io.Fonts->AddFontFromFileTTF(f.bytes, 16.0f, nullptr, io.Fonts->GetGlyphRangesDefault());
...
ImFont *GuiUtils::GetFont(FontSize size)
{
...
size_t GuiUtils::GetPartyHealthbarHeight()
{
    const auto interfacesize =
        static_cast<GW::Constants::InterfaceSize>(GetPreference(GW::UI::Preference_InterfaceSize));
