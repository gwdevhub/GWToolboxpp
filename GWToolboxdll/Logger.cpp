...
{
    SetMessageColor(CHAN_WARNING, 0xFFFFFF44); // warning
    SetMessageColor(CHAN_INFO, 0xFFFFFFFF);    // info
    SetMessageColor(CHAN_ERROR, 0xFFFF4444);   // error
}
...
LONG WINAPI Log::GenerateDump(EXCEPTION_POINTERS *pExceptionPointers)
{
    ...
    bMiniDumpSuccessful = MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(),
                                            hDumpFile, MiniDumpWithDataSegs, &ExpParam, nullptr, nullptr);
    ...
    if (bMiniDumpSuccessful) {
        ...
        MessageBoxW(nullptr, buf, L"GWToolbox++ Crash!", 0);
    } else {
        MessageBoxW(nullptr,
