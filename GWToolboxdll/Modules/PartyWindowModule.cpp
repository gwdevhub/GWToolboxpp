...
bool PartyWindowModule::ShouldRemoveAgentFromPartyWindow(uint32_t agent_id) {
    ...
    for (size_t i = 0; i < allies_added_to_party.size(); i++) {
        ...
        return a->allegiance == static_cast<uint8_t>(GW::Constants::Allegiance::Enemy) || a->GetIsDead() || a->GetIsDeadByTypeMap();
    }
    return false;
}
...
bool PartyWindowModule::ShouldAddAgentToPartyWindow(GW::Agent* _a) {
    ...
    if (!a || !a->IsNPC())
        return false;
    if (a->GetIsDead() || a->GetIsDeadByTypeMap() || a->allegiance == static_cast<uint8_t>(GW::Constants::Allegiance::Enemy))
...
GW::AgentLiving* PartyWindowModule::PendingAddToParty::GetAgent() {
    GW::AgentLiving* a = static_cast<GW::AgentLiving*>(GW::Agents::GetAgentByID(agent_id));
    if (!a || !a->GetIsLivingType() || a->player_number != player_number
        || a->allegiance == static_cast<uint8_t>(GW::Constants::Allegiance::Enemy))
        return nullptr;
