...
{
    return LoadFile(std::filesystem::path(a_pwszFile));
}
...
void ToolboxIni::LoadBuffer(std::string_view buf) {
    Reset();
...
// ===========================================================================
// ToolboxIni – section name cache
// ===========================================================================

void ToolboxIni::rebuildSectionCache() const {
    m_sectionNameCache.clear();
...
std::vector<ToolboxIni::SectionSpan> ToolboxIni::splitSections(std::string_view buf) {
...
void ToolboxIni::parseSection(FastIniSection& sec, std::string_view text) const {
    while (!text.empty()) {
        auto nl = text.find('\n');
        std::string_view line = (nl == std::string_view::npos)
...
bool ToolboxIni::LoadFileRaw(const std::filesystem::path& path)
{
#ifdef _WIN32
    ...
    // Strip UTF-8 BOM
    if (view.size() >= 3 && (unsigned char)view[0] == 0xEF && (unsigned char)view[1] == 0xBB && (unsigned char)view[2] == 0xBF) {
        view = view.substr(3);
    }
...
// ===========================================================================
// ToolboxIni – section access
// ===========================================================================

FastIniSection* ToolboxIni::GetSection(std::string_view name) {
    auto it = m_sections.find(std::string(name));
    return it != m_sections.end() ? &it->second : nullptr;
}

const FastIniSection* ToolboxIni::GetSection(std::string_view name) const {
    auto it = m_sections.find(std::string(name));
    return it != m_sections.end() ? &it->second : nullptr;
}

FastIniSection& ToolboxIni::GetOrCreateSection(std::string_view name) {
...
// ===========================================================================
// ToolboxIni – CSimpleIni-compatible surface
// ===========================================================================

const char* ToolboxIni::GetValue(const char* section, const char* key, const char* def) const {
    const FastIniSection* s = GetSection(section); return s ? s->GetValue(key, def) : def;
}
long ToolboxIni::GetLongValue(const char* section, const char* key, long def) const {
    const FastIniSection* s = GetSection(section); return s ? s->GetLong(key, def) : def;
}
double ToolboxIni::GetDoubleValue(const char* section, const char* key, double def) const {
    const FastIniSection* s = GetSection(section); return s ? s->GetDouble(key, def) : def;
}
bool ToolboxIni::GetBoolValue(const char* section, const char* key, bool def) const {
    const FastIniSection* s = GetSection(section); return s ? s->GetBool(key, def) : def;
}

SI_Error ToolboxIni::SetValue(const char* section, const char* key, const char* value,
                               const char*, bool) {
...
SI_Error ToolboxIni::SetLongValue(const char* section, const char* key, long value,
...
SI_Error ToolboxIni::SetDoubleValue(const char* section, const char* key, double value,
                                     const char*, bool) {
    auto [sec, secInserted] = m_sections.try_emplace(std::string(section));
    if (secInserted) m_sectionCacheDirty = true;
...
SI_Error ToolboxIni::SetBoolValue(const char* section, const char* key, bool value,
                                   const char*, bool) {
    auto [sec, secInserted] = m_sections.try_emplace(std::string(section));
    if (secInserted) m_sectionCacheDirty = true;
...
bool ToolboxIni::GetAllValues(const char* section, const char* key, TNamesDepend& out) const {
    const FastIniSection* s = GetSection(section);
    return s && s->GetAllValues(key, out);
}

bool ToolboxIni::KeyExists(const char* section, const char* key) const {
    const FastIniSection* s = GetSection(section);
    return s && s->HasKey(key);
}

bool ToolboxIni::SectionExists(const char* section) const {
    return GetSection(section) != nullptr;
}

bool ToolboxIni::Delete(const char* section, const char* key, bool) {
...
void ToolboxIni::GetAllSections(TNamesDepend& out) const {
    if (m_sectionCacheDirty) rebuildSectionCache();
    for (const char* p : m_sectionNameCache)
        out.push_back(SI_Entry(p));
}

bool ToolboxIni::GetAllKeys(const char* section, TNamesDepend& out) const {
    const FastIniSection* s = GetSection(section);
    if (!s) return false;
    s->GetAllKeys(out);
    return true;
