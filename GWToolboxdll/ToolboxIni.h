...
// ---------------------------------------------------------------------------
// ToolboxIni – a fast INI parser designed for GWToolbox++.
//
...
//
// pItem    → points into string storage owned by ToolboxIni; valid until the
//            next Reset(), LoadFile(), or destruction of the ToolboxIni object.
...
// ---------------------------------------------------------------------------
// ToolboxIni – the document.
// ---------------------------------------------------------------------------
class ToolboxIni {
public:
    explicit ToolboxIni(bool a_bIsUtf8 = false, bool a_bMultiKey = false, bool a_bMultiLine = false)
        : m_multiKey(a_bMultiKey)
    {
        assert(!a_bIsUtf8    && "ToolboxIni: UTF-8 mode is not supported");
        assert(!a_bMultiLine && "ToolboxIni: multi-line mode is not supported");
    ...
    // Returns SI_OK if file exists and was read successfully (3 retries).
    ...
    // Path that was last successfully loaded from disk.
...
private:
    ...
    bool LoadFileRaw(const std::filesystem::path& path);
};
