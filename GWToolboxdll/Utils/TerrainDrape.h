...
// Terrain-draping helpers shared by the in-world overlay modules (danger rings, loot beacons,
...
namespace TerrainDrape {
    ...
    // Surface altitude at (x,y), preferring `zplane`, else the plane closest to `ref`.
    float DrapeZ(float x, float y, uint32_t zplane, uint32_t n_planes, float ref);

    // Surface altitude at (x,y): the given plane if it has data, else the highest surface
    // across all planes. Returns 0.f when nothing has data.
