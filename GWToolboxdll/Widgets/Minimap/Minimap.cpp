...
void Minimap::SelectTarget(const GW::Vec2f pos)
{
    ...
    for (const auto* agent : *agents) {
        ...
        const auto agent_is_locked_chest = agent->GetIsGadgetType() && wcseq(GW::Agents::GetAgentEncName(agent->agent_id), GW::EncStrings::LockedChest);
        if (!agent->GetIsLivingType()) {
            if (!agent_is_locked_chest && !target_gadgets_on_ctrl_click) {
                continue;
            }
            if (!GW::Agents::GetAgentMatchesFlags(agent, GW::TargetFilter::AnyLiving | GW::TargetFilter::Gadgets)) {
                continue;
            }
        } else if (agent->GetAsAgentLiving()->GetIsDead()) {
            continue;
        }
