...
// each corner's offset from the ring's centre in world units (NOT a 0..1 texture UV) - the pixel shader
// turns length(local) back into a radius to shade the ring, so no texture/sampler is involved at all.
