...
#include "Minimap/Shaders/icon_tint_ps.h"
...
void MissionMapWidget::Draw(IDirect3DDevice9* dx_device)
{
    ...
    // Desaturate the icon texture and apply the affiliation tint via a pixel
    // shader: sample the texture, convert to greyscale, then multiply by the
    // vertex diffuse colour so the icon shape is preserved with the correct hue.
        ...
        [](const ImDrawList*, const ImDrawCmd* cmd) {
            ...
            if (!icon_tint_shader) {
                dev->CreatePixelShader(
                    reinterpret_cast<const DWORD*>(&icon_tint_ps),
                    &icon_tint_shader);
            }
            dev->SetPixelShader(icon_tint_shader);
        },
        dx_device);
