...
void NavPmapRenderer::LoadSettings(const ToolboxIni* ini, const char* section)
{
...
void NavPmapRenderer::SaveSettings(ToolboxIni* ini, const char* section) const
{
...
void NavPmapRenderer::DrawSettings()
{
    bool confirm = false;
...
void NavPmapRenderer::Initialize(IDirect3DDevice9* device)
{
    ...
    if(!builder.Build(path_map))
    {
        Log::Error("Could not build the navmesh, please try different settings");
    }
}

extern volatile bool navmesh_building;

void NavPmapRenderer::Render(IDirect3DDevice9* device)
{
    if (!initialized) {
        initialized = true;
        Initialize(device);
