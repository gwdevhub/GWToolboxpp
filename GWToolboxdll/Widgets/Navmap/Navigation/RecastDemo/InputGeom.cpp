...
bool InputGeom::load(rcContext* ctx, const std::string& filepath)
{
	...
	//std::transform(extension.begin(), extension.end(), extension.begin(), (char)tolower);
    std::transform(extension.begin(), extension.end(), extension.begin(), [](int c) -> char { return static_cast<char>(::tolower(c)); });
