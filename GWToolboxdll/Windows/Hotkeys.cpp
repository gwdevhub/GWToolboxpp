    ...
    switch (HasProfession()) {
        ...
        default: {
            ...
            for (size_t i = 1; i < _countof(prof_ids); i++) {
                ...
                if (!prof_ids_written)
                    format = "%s";
                prof_ids_written += snprintf(&prof_ids_buf[prof_ids_written], _countof(prof_ids_buf) - prof_ids_written, format, GetProfessionAcronym(static_cast<GW::Constants::Profession>(i)).c_str());
            }
