...
namespace {
    ...
    constexpr size_t formation_hero_count = std::size(formation_hero_names);
    ...
    // Returns the display name for a hero, resolving merc names dynamically
    const char* GetFormationHeroName(size_t hero_id) {
        if (hero_id >= formation_hero_count) return "";
        ...
        if (hero_id >= 28 && hero_id <= 35) {
            ...
            // Find this merc's agent_id from the party
                ...
                if (heroes.valid()) {
                    for (size_t i = 0; i < heroes.size(); i++) {
                        if (heroes[i].owner_player_id == me->login_number &&
                            ...
                            heroes[i].agent_id) {
                            // Only re-resolve if agent changed
...
void HotkeyHeroFormation::DrawFormationCanvas()
{
    // Zoom dropdown
    ...
    // Capture position — ImDrawList API uses screen coordinates
    ...
    // Draw background before InvisibleButton (matches ImGui demo canvas pattern)
    ...
    // InvisibleButton captures interactions and advances layout cursor
    ...
    for (int i = 0; i < 7; i++) {
        ...
        draw_list->AddText(ImVec2(sx - 3, sy - 5), IM_COL32(0, 0, 0, 255), label);
    }

    draw_list->PopClipRect();
}

void HotkeyHeroFormation::DrawHeroAssignment()
{
    ...
    // 3-column hero checkboxes (skip index 0 = NoHero)
    constexpr int columns = 3;
    if (ImGui::BeginTable("##heroes", columns, ImGuiTableFlags_None)) {
        for (size_t h = 1; h < formation_hero_count; h++) {
            const char* hero_name = GetFormationHeroName(h);
            if (hero_name[0] == '\0') continue; // skip empty
...
void HotkeyHeroFormation::Execute()
{
    ...
    for (int i = 0; i < 7; i++) {
        ...
        // Snap to walkable terrain if position is inaccessible
        Pathing::FindClosestPositionOnTrapezoid(pos);

        // Flag each hero assigned to this slot
        for (size_t h = 1; h < 64; h++) {
            ...
            if (party_idx < 0) continue; // hero not in party, skip
