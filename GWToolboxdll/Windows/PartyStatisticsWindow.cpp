...
*/
                       ...
                       const bool count_attributed)
    {
        ...
        if (is_healing) {
            ...
            if (actual_amount > 0) {
                AddPartyMemberCombatDamage(party_member, actual_amount, GetTimeMs());
            }
        }
        if (count_attributed) {
            skill->attributed++;
    ...
    void DrawPartyMember(PartyMember& party_member)
    {
        ...
        if (ImGui::CollapsingHeader(header_label, ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::Text("DPS: %u   Heal: %u", dps, total_healing);
            if (HasSkill(party_member, GW::Constants::SkillID::Mind_Burn)) {
