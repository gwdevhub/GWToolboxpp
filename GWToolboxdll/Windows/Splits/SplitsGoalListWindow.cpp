...
// 0 = "MM:SS.CC", 1 = "H:MM:SS.CC", 2 = "HH:MM:SS.CC" — matches FormatTime's own digit growth.
static int TimeDigitTier(double seconds)
{
    const int h = static_cast<int>(seconds) / 3600;
    return h >= 10 ? 2 : (h >= 1 ? 1 : 0);
}

// Measured once per tier (font/DPI don't change mid-session) instead of every frame — widens the goal row's time columns only once the clock actually needs the extra digit, instead of always reserving room for the worst case.
static float TimeColumnWidth(int tier, bool with_sign)
{
    ...
    if (slot < 0.f) {
        ...
        slot = ImGui::CalcTextSize(with_sign ? signed_[tier] : plain[tier]).x + 12.f;
    }
    return slot;
}
...
void SplitsGoalListWindow::Draw(SplitsWindow& plugin)
{
    ...
    // Header clock — shows whichever time the profile is set to display, plus run controls.
    {
        ...
        // Computed here (not down by the buttons themselves) so the clock text below can be clamped against it on a narrow window.
        ...
        // Centering can otherwise push the clock text right underneath the buttons once the window's too narrow to fit both.
        ...
        const ImVec4 real_col = ImGui::ColorConvertU32ToFloat4(plugin.ColorRealTime());
        const ImVec4 game_col = ImGui::ColorConvertU32ToFloat4(plugin.ColorGameTime());
        ...
        // Run controls + settings gear, right-aligned on the same row as the clock (widths/button_x computed above, alongside the clock-text clamp).
    ...
    if (plugin.RunFailed()) {
        ...
        const float cx = ImGui::GetCursorPosX() + (avail - tw) * 0.5f;
        ImGui::SetCursorPosX(cx > ImGui::GetCursorPosX() ? cx : ImGui::GetCursorPosX());
        ImGui::TextColored(ImGui::ColorConvertU32ToFloat4(plugin.ColorPbBehind()), "%s", kMsg);
        ImGui::Separator();
    }
...
void SplitsGoalListWindow::DrawGoalRow(const GoalEntry& g, const GoalClock& clock,
                                        ...
                                        const SplitsProfile& profile, SplitsWindow& plugin)
{
    const bool done = (g.status == GoalStatus::Completed || g.status == GoalStatus::Failed);

    ImVec4 color;
    if (g.status == GoalStatus::Failed)         color = ImGui::ColorConvertU32ToFloat4(plugin.ColorPbBehind());
    else if (g.status == GoalStatus::Completed) color = ImGui::ColorConvertU32ToFloat4(plugin.ColorCompleted());
    // added to support current highlighting for multiple goals at a time
    else if (is_current || g.status == GoalStatus::Started) color = ImGui::ColorConvertU32ToFloat4(plugin.ColorActive());
    ...
    const ImVec4 real_col   = ImGui::ColorConvertU32ToFloat4(plugin.ColorRealTime());
    const ImVec4 game_col   = ImGui::ColorConvertU32ToFloat4(plugin.ColorGameTime());
    const ImVec4 ahead_col  = ImGui::ColorConvertU32ToFloat4(plugin.ColorPbAhead());
    const ImVec4 behind_col = ImGui::ColorConvertU32ToFloat4(plugin.ColorPbBehind());
    ...
    if (g.display_style == GoalEntry::DisplayStyle::Dynamic || profile.dynamic_by_default) {
        // Start/End/Duration for THIS goal alone, not "vs previous row" — needed for independent-start goals (Quest pickup, Deep's parallel rooms).
        ...
        const int tier = TimeDigitTier(std::max(clock.RealTime(), clock.GameTime()));
        ...
        if (show_real) {
            draw_or_dash(real_col, g.split.real_time - g.start_real_time, have_duration);
            if (have_duration && profile.show_segment_pb)
                DrawPBDelta(g.split.real_time - g.start_real_time, pb_seg_real, ahead_col, behind_col);
        }
        if (show_game) {
            draw_or_dash(game_col, g.split.game_time - g.start_game_time, have_duration);
            if (have_duration && profile.show_segment_pb)
                DrawPBDelta(g.split.game_time - g.start_game_time, pb_seg_game, ahead_col, behind_col);
        }
...
void SplitsGoalListWindow::DrawRecentRunsSection(SplitsWindow& plugin)
{
    const SplitsProfile& profile = plugin.ActiveProfile();
    if (!profile.show_recent_runs) return;
    const auto& runs = plugin.RecentRuns();
    ...
    for (size_t i = 0; i < runs.size(); ++i) {
        ...
        char dbuf[16] = "";
        if (run.utc_start > 0) {
            const time_t t = static_cast<time_t>(run.utc_start);
            tm tm_buf{};
            localtime_s(&tm_buf, &t);
            snprintf(dbuf, sizeof(dbuf), "%04d-%02d-%02d", tm_buf.tm_year + 1900, tm_buf.tm_mon + 1, tm_buf.tm_mday);
        }
        char label[96];
        snprintf(label, sizeof(label), "%s  %s%s", tbuf, dbuf, run.failed ? "  [Failed]" : "");

        if (ImGui::CollapsingHeader(label)) {
            for (const auto& g : run.goals) {
                const ImVec4 col = g.completed
                    ? ImGui::ColorConvertU32ToFloat4(plugin.ColorCompleted())
                    : ImGui::GetStyleColorVec4(ImGuiCol_TextDisabled);
                char gbuf[32];
                FormatTime(gbuf, sizeof(gbuf), gt ? g.game_time : g.real_time);
...
void SplitsGoalListWindow::DrawSettings(SplitsWindow& plugin)
{
    ...
    // SC's lists are tool-generated (not meant for hand-editing) while Manual/Running are user-authored — the one place the two families diverge in shape.
...
void SplitsGoalListWindow::DrawTimeAndBehaviorColumn(SplitsWindow& plugin)
{
    ...
    // Dynamic-style rows (SC, and Running's legs) have no separate "total"/"seg" columns to toggle — just their one Duration delta. Matches DrawGoalRow's own Dynamic condition (display_style == Dynamic || dynamic_by_default) — Running's legs are always display_style == Dynamic (see DrawMapBatchPicker's preserve_order build), hence checking sequential_route here too.
    if (!p.dynamic_by_default && !p.sequential_route) {
        ...
        ImGui::Checkbox("Show duration delta", &p.show_segment_pb);
    }
    ...
    ImGui::Checkbox("Auto-fail on leaving a zone early", &p.auto_fail_on_rezone);
    if (ImGui::IsItemHovered())
        ImGui::SetTooltip("If you leave a zone before accomplishing all goals tied to that zone, the run will fail.");
}
...
void SplitsGoalListWindow::DrawKeybindsAndColorsColumn(SplitsWindow& plugin)
{
    ...
    ImGui::Spacing();
    // Shared across all profiles now — see ColorCompleted() etc. on SplitsWindow.
    ImGui::ColorButtonPicker("Completed##sc", &plugin.ColorCompleted()); ImGui::SameLine(); ImGui::TextUnformatted("Completed");
    ImGui::SameLine(0, 12.f);
    ImGui::ColorButtonPicker("Current##sc",   &plugin.ColorActive());    ImGui::SameLine(); ImGui::TextUnformatted("Current");
    ImGui::ColorButtonPicker("Real##sc",      &plugin.ColorRealTime());  ImGui::SameLine(); ImGui::TextUnformatted("Real");
    ImGui::SameLine(0, 12.f);
    ImGui::ColorButtonPicker("Game##sc",      &plugin.ColorGameTime());  ImGui::SameLine(); ImGui::TextUnformatted("Game");
    ImGui::ColorButtonPicker("Ahead##sc",     &plugin.ColorPbAhead());   ImGui::SameLine(); ImGui::TextUnformatted("Ahead");
    ImGui::SameLine(0, 12.f);
    ImGui::ColorButtonPicker("Behind##sc",    &plugin.ColorPbBehind());  ImGui::SameLine(); ImGui::TextUnformatted("Behind");
    ImGui::Spacing();
}
...
void SplitsGoalListWindow::DrawSCGoalsSummary(SplitsWindow& plugin)
{
    ...
    if (!list->name.empty()) {
        ImGui::SameLine();
        // Clears the active list so ApplySCAutoLoadPreset's anchor-map_id match no longer sees it as "already correct" and rebuilds fresh on the next map entry.
        if (ImGui::SmallButton("Use Default##sc")) plugin.NewActiveList("");
    }
}
...
void SplitsGoalListWindow::DrawEditableGoalsList(SplitsWindow& plugin)
{
    ...
    for (int i = 0; i < static_cast<int>(list->goals.size()) && !erased && !moved; ++i) {
        ...
        ImGui::SameLine();
        // Nested headers don't exist, so only non-header rows ever show the indent hint.
        if (!g.is_header && g.indent > 0) { ImGui::TextDisabled("|"); ImGui::SameLine(0, 2); }
        ImGui::TextUnformatted(g.label.c_str());
        ImGui::SameLine();

        // indent/grouping is set programmatically (e.g. SC's presets), not exposed to users here — keep it simple.
        ...
        if (i == first_goal_idx) {
            if (plugin.ActiveProfile().sequential_route) {
...
void SplitsGoalListWindow::DrawStandardAddGoalForm(SplitsWindow& plugin)
{
    ...
    // Running is a route — just picks which towns/explorables it enters/leaves, in order.
    ...
    const TriggerOpt* trigger_opts      = is_sc ? trigger_opts_sc : is_running ? trigger_opts_running : trigger_opts_full;
    int               trigger_opts_count = is_sc ? static_cast<int>(std::size(trigger_opts_sc))
                                          : is_running ? static_cast<int>(std::size(trigger_opts_running))
                                                          : static_cast<int>(std::size(trigger_opts_full));
    // True when edit_trigger_type_ is one of opts' own entries — used below to reset a leftover Manual-only selection to a valid default when SC/Running becomes active, instead of listing each array's entries by hand (which can silently desync from the arrays above as options are added/removed).
    auto is_valid_selection = [&](const TriggerOpt* opts, int count) {
        for (int i = 0; i < count; ++i)
            if (opts[i].type_int == edit_trigger_type_) return true;
        return false;
    };
    if (is_sc && !is_valid_selection(trigger_opts_sc, static_cast<int>(std::size(trigger_opts_sc))))
    ...
    if (is_running && !is_valid_selection(trigger_opts_running, static_cast<int>(std::size(trigger_opts_running))))
        edit_trigger_type_ = trigger_opts_running[1].type_int;
...
void SplitsGoalListWindow::DrawTitlePicker(SplitsWindow& plugin)
{
    ...
    // Built once, lazily: every non-deprecated title, Copied from TitleTrackerWidget
...
namespace {
// Shared by DrawTownBatchPicker/DrawExplorableBatchPicker — consolidated from two near-identical ~180-line copies so a future column/filter change only needs to happen once.
...
// type_ok's rank breaks ties on the same name_id (Town never sets rank, so ties keep whichever map id was encountered first).
    ...
    const std::function<bool(const GW::AreaInfo&, int&)>& type_ok)
{
    ...
    for (int id = 1; id < static_cast<int>(MapID::Count); ++id) {
        ...
        // skip duplicates with empty string+mapid
        if (Resources::GetMapName(mid)->string().empty()) continue;
                        ...
                        const std::vector<BatchColumn>& columns)
{
    ...
    // Running wants goals added in the order boxes were checked (an actual route), not the region/name sort Manual/SC want from a bulk pick.
    const bool preserve_order = plugin.ActiveProfile().sequential_route;
    ...
    // AreaInfo never changes mid-session, so cache per (picker, campaign) instead of rescanning ~1300 MapIDs every frame.
    static std::unordered_map<std::string, std::vector<BatchMapRow>> row_cache;
    auto cached_rows = [&](const std::string& key, const std::function<bool(const GW::AreaInfo&)>& region_ok) -> const std::vector<BatchMapRow>& {
        const auto it = row_cache.find(key);
        if (it != row_cache.end()) return it->second;
        return row_cache.emplace(key, BuildBatchMapRows(region_ok, type_ok)).first->second;
    };
    ...
    if (ImGui::Button(add_lbl)) {
        ...
        if (preserve_order) {
            // Running: exact check order, this is a route, not a bulk pick.
        ...
        for (const auto& item : items) {
            ...
            if (preserve_order) {
                // One goal per leg — the Enter type becomes its start_trigger (see GoalEngine Pass 1), its own trigger is the matching Exit, so Duration shows time-spent same as SC's checkpoints.
                g.label                 = item.name;
...
void SplitsGoalListWindow::DrawTownBatchPicker(SplitsWindow& plugin)
{
    ...
    // Running: one box per row — Add builds a single start_trigger/trigger pair goal (see DrawMapBatchPicker's preserve_order branch).
    ...
    const bool is_running = plugin.ActiveProfile().sequential_route;
    DrawMapBatchPicker(plugin, "town", town_filter_buf_, sizeof(town_filter_buf_),
                       batch_town_checked_, batch_town_order_, type_ok,
                       is_running ? std::vector<BatchColumn>(std::begin(kColumnsRunning), std::end(kColumnsRunning))
...
void SplitsGoalListWindow::DrawExplorableBatchPicker(SplitsWindow& plugin)
{
    ...
    // Running has no use for VQ, and a leg is just "pass through" — one box builds a single start_trigger/trigger pair goal.
    static const BatchColumn kColumnsRunning[] = {
    ...
    DrawMapBatchPicker(plugin, "exp", exp_filter_buf_, sizeof(exp_filter_buf_),
                       batch_exp_checked_, batch_exp_order_, type_ok,
                       is_running ? std::vector<BatchColumn>(std::begin(kColumnsRunning), std::end(kColumnsRunning))
                                  : std::vector<BatchColumn>(std::begin(kColumns), std::end(kColumns)));
}
...
void SplitsGoalListWindow::DrawEliteAreaBatchPicker(SplitsWindow& plugin)
{
    ...
    if (plugin.ActiveProfile().dynamic_by_default) {
        ...
        // DoA's rotation is spawn-dependent (see doa_preset_cache_), so unlike the areas above there's no single "right" list to auto-generate — these just let you preview/pre-build one of the 4 known rotations. If the real starting zone turns out different once you're actually in DoA, splits still fire and time correctly (each zone's own start_trigger is checked independent of list position), only the displayed order would read differently than what actually happened.
        static const char* kDoAZoneNames[4] = { "Foundry", "City", "Veil", "Gloom" };
        const auto& doa_cache = plugin.DoAPresetCache();
        for (int i = 0; i < 4; ++i) {
            char label[48];
            snprintf(label, sizeof(label), "Domain of Anguish (starts: %s)", kDoAZoneNames[i]);
            if (ImGui::Selectable(label)) {
                plugin.SetActiveList(doa_cache[static_cast<size_t>(i)]);
                snprintf(list_name_buf_, sizeof(list_name_buf_), "Domain of Anguish");
            }
        }
        ImGui::EndChild();
        return;
