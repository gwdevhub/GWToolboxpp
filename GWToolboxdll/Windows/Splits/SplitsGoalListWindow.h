...
// Per-profile settings live on SplitsProfile (plugin.ActiveProfile()); colors are global on SplitsWindow itself.
// ---------------------------------------------------------------------------
...
private:
    ...
    void DrawGoalRow(const GoalEntry& g, const GoalClock& clock, int index,
                     ...
                     const struct SplitsProfile& profile, SplitsWindow& plugin);
    void DrawRecentRunsSection(SplitsWindow& plugin);
    ...
    // Running only: records check order so its route can be added in click order instead of the region/name sort Manual/SC want.
    std::vector<std::pair<int, uint8_t>> batch_exp_order_;
    char                   exp_filter_buf_[128] = {};

    std::map<int, uint8_t> batch_town_checked_;
    std::vector<std::pair<int, uint8_t>> batch_town_order_;
    char                   town_filter_buf_[128] = {};

    std::set<int> batch_dungeon_checked_;
    char          dungeon_filter_buf_[128] = {};

    // Elite area picker — keyed by checkpoint array index, not param1 (Urgoz/Deep mix trigger types whose param1/pattern values could otherwise collide).
