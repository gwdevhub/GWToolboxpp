...
#cs
	...
	Known issues:
	The SciTE files have not be updated yet but will be in the next minor release.
...
Func _Resource_GetAsRaw($sResNameOrID, $iResType = $RT_RCDATA, $iResLang = Default, $sDllOrExePath = Default)
	Return __Resource_Get($sResNameOrID, $iResType, $iResLang, $sDllOrExePath, $RT_RCDATA)
...
Func _Resource_SetBitmapToCtrlID($iCtrlID, $hHBITMAP, $bResize = Default)
	Return __Resource_SetToCtrlID($iCtrlID, $hHBITMAP, $RT_BITMAP, False, $bResize)
...
Func _Resource_SetCursorToCtrlID($iCtrlID, $hCursor, $bResize = Default)
	Return __Resource_SetToCtrlID($iCtrlID, $hCursor, $RT_CURSOR, False, $bResize)
...
Func _Resource_SetIconToCtrlID($iCtrlID, $hIcon, $bResize = Default)
	Return __Resource_SetToCtrlID($iCtrlID, $hIcon, $RT_ICON, False, $bResize)
				...
				If $iError = $RESOURCE_ERROR_NONE And $iLength > 0 Then
					...
					If $bReturn Then
						If $__WINVER >= 0x0600 Then
							$bReturn = _WinAPI_DeleteObject($hHBITMAP) ; Delete if Vista or above.
							$vReturn = $bReturn
						Else
...
Func __Resource_Destroy($pResource, $iResType)
	...
	Switch $iResType
		Case $RT_ANICURSOR, $RT_CURSOR
			$bReturn = _WinAPI_DeleteObject($pResource)
			If Not $bReturn Then
				$bReturn = _WinAPI_DestroyCursor($pResource)
			EndIf
		Case $RT_BITMAP
			$bReturn = _WinAPI_DeleteObject($pResource)
		Case $RT_FONT
		...
		Case $RT_ICON
			$bReturn = _WinAPI_DeleteObject($pResource)
			If Not $bReturn Then
				$bReturn = _WinAPI_DestroyIcon($pResource)
			EndIf
		Case $RT_MENU
			$bReturn = _GUICtrlMenu_DestroyMenu($pResource)
		Case $RT_STRING
			$bReturn = True ; No action required.
		Case $RESOURCE_RT_BITMAP
			$bReturn = _GDIPlus_BitmapDispose($pResource)
