...
{
#ifndef _NDEBUG
    static_assert(sizeof(m_ErrorBuffer) >= CURL_ERROR_SIZE,
...
void CurlEasy::SetUrl(const Protocol proto, const char* url)
{
...
void CurlEasy::SetPort(const uint16_t port)
{
...
void CurlEasy::SetMethod(const HttpMethod method)
{
    switch (method) {
        case HttpMethod::Get:
...
void CurlEasy::SetHeaders(const std::initializer_list<ParamField> headers)
{
    for (const ParamField header : headers) {
...
void CurlEasy::SetTimeoutMs(const int timeout_ms)
{
    CHECK_CURL_EASY_SETOPT(this, CURLOPT_TIMEOUT_MS, static_cast<long>(timeout_ms));
}

void CurlEasy::SetTimeoutSec(const int timeout_sec)
{
...
void CurlEasy::SetProxy(const char* url, const uint16_t port)
{
    CHECK_CURL_EASY_SETOPT(this, CURLOPT_PROXY, url);
...
void CurlEasy::SetBufferSize(const size_t size)
{
    assert(1024 <= size && size <= CURL_MAX_READ_SIZE);
    CHECK_CURL_EASY_SETOPT(this, CURLOPT_BUFFERSIZE, static_cast<long>(size));
}

void CurlEasy::SetPostContent(const std::string& content, const ContentFlag flag)
{
    return SetPostContent(content.c_str(), content.size(), flag);
}

void CurlEasy::SetPostContent(const char* content, const size_t Size, const ContentFlag flag)
{
...
void CurlEasy::SetUploadBuffer(const std::string& content, const ContentFlag flag)
{
    return SetUploadBuffer(content.c_str(), content.size(), flag);
}

void CurlEasy::SetUploadBuffer(const char* content, const size_t size, const ContentFlag flag)
{
    assert(content && size);
    if (flag == ContentFlag::Copy) {
...
void CurlEasy::SetUploadFile(FILE* file, const size_t size)
{
    m_UploadFile = file;
    CHECK_CURL_EASY_SETOPT(this, CURLOPT_UPLOAD, 1L);
...
void CurlEasy::UpdateStatus(const int CurlStatus)
{
    m_StatusCode = 0;
    long ResponseStatus;
...
const char* CurlEasy::GetProtocolPrefix(const Protocol proto)
{
    switch (proto) {
        case Protocol::Ftp:
            return "ftp://";
...
const char* CurlEasy::GetHttpMethodName(const HttpMethod method)
{
    switch (method) {
        case HttpMethod::Get:
            return "GET";
...
void CurlEasy::OnHeader(const char* bytes, const size_t count)
{
    m_Header.append(bytes, count);
}

void CurlEasy::OnContent(const char* bytes, const size_t count)
{
    m_Content.append(bytes, count);
}

size_t CurlEasy::WriteCallback(const char* buffer, const size_t size, const size_t nitems, void* userdata)
{
    const auto easy = static_cast<CurlEasy*>(userdata);
...
size_t CurlEasy::HeaderCallback(const char* buffer, const size_t size, const size_t nitems, void* userdata)
{
    const auto easy = static_cast<CurlEasy*>(userdata);
    const size_t count = size * nitems;
...
size_t CurlEasy::ReadFileCallback(char* buffer, const size_t size, const size_t nitems, void* userdata)
{
    const auto easy = static_cast<CurlEasy*>(userdata);
    return fread(buffer, size, nitems, easy->m_UploadFile);
}

size_t CurlEasy::ReadBufferCallback(char* buffer, const size_t size, const size_t nitems, void* userdata)
{
    const auto easy = static_cast<CurlEasy*>(userdata);
    UploadBuffer* stream = &easy->m_UploadBuffer;
    const size_t count = size * nitems;
    const size_t bytes = std::min<size_t>(count, stream->size - stream->rpos);
    memcpy(buffer, stream->data, bytes);
    stream->rpos += bytes;
    return bytes;
}

void CurlEasy::HandleOptionError(CurlEasy* easy, const int code, const char* file, const unsigned line)
{
    if (code != CURLE_OK) {
        file = file ? file : "<file>";
...
void CurlMulti::AddHandle(CurlEasy* easy) const
{
...
void CurlMulti::RemoveHandle(CurlEasy* easy) const
{
    assert(easy->m_MultiHandle);

    const CURLMcode code = curl_multi_remove_handle(m_Handle, easy->m_Handle);
    assert(code == CURLM_OK);
    if (code == CURLM_OK) {
...
void CurlMulti::Perform() const
{
    int running_handles;
    const CURLMcode code = curl_multi_perform(m_Handle, &running_handles);
    if (code != CURLM_OK) {
