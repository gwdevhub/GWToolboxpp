...
public:
    ...
    void SetUploadBuffer(const std::string& content, ContentFlag flag);
...
#ifndef _NDEBUG
    const char* GetErrorStr() const { return m_ErrorBuffer; }
#endif
...
private:
    static size_t WriteCallback(const char* buffer, size_t size, size_t nitems, void* userdata);
...
public:
    ...
    CURLM* GetHandle() const { return m_Handle; }

    // AddHandle, RemoveHandle are as thread-safe as curl_multi_add_handle & curl_multi_remove_handle
    void AddHandle(CurlEasy* Handle) const;
    void RemoveHandle(CurlEasy* Handle) const;
