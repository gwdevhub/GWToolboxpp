...
std::string ProjectileIndicator::GetSkillName(const GW::Constants::SkillID id)
{
    if (id == GW::Constants::SkillID::No_Skill) {
        return "Auto-Attack";
    }
    if (const auto it = decodedSkillNames.find(id); it != decodedSkillNames.end()) {
        return PluginUtils::WStringToString(it->second);
    }

    const auto skillData = GW::SkillbarMgr::GetSkillConstantData(id);
    if (!skillData || static_cast<uint32_t>(id) > static_cast<uint32_t>(GW::Constants::SkillID::Dhuums_Covenant_Broken)) {
        return {};
    }

    wchar_t encoded[8]{};
    if (!GW::UI::UInt32ToEncStr(skillData->name, encoded, _countof(encoded))) {
        return {};
    }

    decodedSkillNames.emplace(id, L"");
    const auto context = new SkillNameDecodeContext{this, id, encoded};
    pendingSkillNameDecodes.push_back(context);
    GW::UI::AsyncDecodeStr(context->encoded.c_str(), &OnSkillNameDecoded, context);
    return {};
}

void ProjectileIndicator::OnSkillNameDecoded(void* context, const wchar_t* decoded)
{
    const auto decode = static_cast<SkillNameDecodeContext*>(context);
    if (decode && decode->owner) {
        decode->owner->CompleteSkillNameDecode(decode, decoded);
    }
}
...
void ProjectileIndicator::Initialize(ImGuiContext* ctx, ImGuiAllocFns fns, HMODULE toolbox_dll)
{
    ...
    GW::StoC::RegisterPacketCallback<GW::Packet::StoC::AgentProjectileLaunched>(&projectileHook, [this](GW::HookStatus* status, GW::Packet::StoC::AgentProjectileLaunched* packet) -> void {
        ...
        RenderingUtils::addEllipseToDraw(packet->destination, ADirection, ellipsisA, ellipsisB, ImGui::ColorConvertFloat4ToU32(color), filled, projectileTimer, this);
    });
}
