...
private:
    struct SkillNameDecodeContext {
        ProjectileIndicator* owner = nullptr;
        GW::Constants::SkillID id = GW::Constants::SkillID::No_Skill;
        std::wstring encoded;
    };

    std::string GetSkillName(GW::Constants::SkillID id);
    void CompleteSkillNameDecode(SkillNameDecodeContext* context, const wchar_t* decoded);
    static void OnSkillNameDecoded(void* context, const wchar_t* decoded);
    ...
    std::unordered_map<GW::Constants::SkillID, std::wstring> decodedSkillNames;
    std::vector<SkillNameDecodeContext*> pendingSkillNameDecodes;
