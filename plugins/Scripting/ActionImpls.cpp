...
namespace {
    ...
    void SafeEquip(GW::Item* item)
    {
        ...
        if (!item || !p || p->GetIsDead() || p->GetIsKnockedDown() || (s && s->cast_array.size() > 0)) return;
        if (p->skill) {
...
/// ------------- GoToTargetAction -------------
std::map<uint32_t, std::vector<bool*>> gotoTargetDialogPoppedUp;
...
ActionStatus ConditionedAction::isComplete() const
{
    ...
    {
        if (currentlyExecutedActions.front()) {
            currentlyExecutedActions.front()->finalAction();
        }
...
void ConditionedAction::finalAction()
{
    if (!currentlyExecutedActions.empty() && currentlyExecutedActions.front()) {
        currentlyExecutedActions.front()->finalAction();
    }
    currentlyExecutedActions.clear();
    Action::finalAction();
...
void PingHardModeAction::initialAction()
{
    ...
    GW::GameThread::Enqueue([pingId = player->agent_id]() {
        Ping_Func((GW::CallTargetType) 0x4, pingId); // 0x4 = HardMode
    });
}
