...
void InstanceInfo::resetRuntime()
{
    ++decodeGeneration;
    decodedAgentNames.clear();
...
{
    if (const auto found = decodedAgentNames.find(id); found != decodedAgentNames.end()) {
        return WStringToString(found->second);
    }
    ...
    decodedAgentNames.emplace(id, L"");
    const auto generation = decodeGeneration;
    AsyncStringDecoder::Decode(encodedName, [this, id, generation](const wchar_t* decoded) {
        if (decoded && generation == decodeGeneration) {
            decodedAgentNames[id] = decoded;
        }
    });
    return "";
}
...
{
    if (const auto found = decodedItemNames.find(item_id); found != decodedItemNames.end()) {
        auto decoded = WStringToString(found->second);
        removeTextInAngledBrackets(decoded);
        return decoded;
    }

    const auto item = GW::Items::GetItemById(item_id);
    if (!item || !item->single_item_name) return "";

    decodedItemNames.emplace(item_id, L"");
    const auto generation = decodeGeneration;
    AsyncStringDecoder::Decode(item->single_item_name, [this, item_id, generation](const wchar_t* decoded) {
        if (decoded && generation == decodeGeneration) {
            decodedItemNames[item_id] = decoded;
        }
    });
    return "";
}
