...
namespace {
    SpeedrunScriptingTools* activeScriptingTools = nullptr;
    ...
    {
        const auto instance = activeScriptingTools;
        if (!instance || argc < 2) {
...
void SpeedrunScriptingTools::clear() 
{
    ...
    for (auto& group : m_groups) {
        for (auto& script : group.scripts) {
            script.triggered = false;
        }
    }
}

SpeedrunScriptingTools::SpeedrunScriptingTools()
{
    can_close = true;
...
void SpeedrunScriptingTools::Draw(IDirect3DDevice9*)
{
    ...
    if (ImGui::Begin("SST Script Toggles###SpeedrunScriptingTools", show_closebutton ? visible : nullptr, GetWinFlags())) {
        ...
        const auto set_script_state = [&](const bool enabled) {
            for (auto& group : m_groups) {
                toggles_changed |= group.enabled != enabled;
                group.enabled = enabled;
                for (auto& script : group.scripts) {
                    toggles_changed |= script.enabled != enabled;
        ...
        const auto draw_script_toggles = [&](std::vector<Script>& scripts) {
            for (auto& script : scripts) {
                ImGui::PushID(static_cast<int>(script.getId()));
                const auto label = script.name.empty() ? "Unnamed script" : script.name.c_str();
                toggles_changed |= ImGui::Checkbox(label, &script.enabled);
                if (std::ranges::any_of(m_currentScripts, [&](const auto& running) { return running.getId() == script.getId(); })) {
                    ImGui::SameLine();
                    ImGui::TextDisabled("[running]");
                }
                if (script.enabledToggleHotkey.keyData) {
                    ImGui::SameLine();
                    ImGui::TextDisabled("(%s)", makeHotkeyDescription(script.enabledToggleHotkey).c_str());
                }
                ImGui::PopID();
            }
        };

        if (ImGui::Button("Enable all")) set_script_state(true);
        ImGui::SameLine();
        if (ImGui::Button("Disable all")) set_script_state(false);

        if (m_groups.empty() && m_scripts.empty()) {
            ImGui::TextDisabled("No scripts configured.");
        }
        if (!m_groups.empty()) {
            ImGui::SeparatorText("Groups");
            for (auto group_index = size_t{0}; group_index < m_groups.size(); ++group_index) {
                auto& group = m_groups[group_index];
                ImGui::PushID(static_cast<int>(group_index));
                const auto label = group.name.empty() ? "Unnamed group" : group.name.c_str();
                toggles_changed |= ImGui::Checkbox(label, &group.enabled);
                if (!group.scripts.empty()) {
                    ImGui::Indent();
                    draw_script_toggles(group.scripts);
                    ImGui::Unindent();
                }
                ImGui::PopID();
            }
        ...
        if (!m_scripts.empty()) {
            ImGui::SeparatorText("Ungrouped scripts");
            draw_script_toggles(m_scripts);
        }
        if (!m_currentScripts.empty()) {
            ImGui::Separator();
            ImGui::TextDisabled("Disabling prevents new launches; running scripts continue until completion or are cleared in SST settings.");
        }
        if (toggles_changed) refreshDisabledKeys();
    }
    ImGui::End();
}
...
bool SpeedrunScriptingTools::WndProc(const UINT Message, const WPARAM wParam, LPARAM lparam)
{
    ...
    switch (Message) {
        ...
        case WM_MBUTTONDBLCLK: {
            ...
            auto enable_state_changed = false;
            if (clearScriptsKey.keyData && clearScriptsKey == pressedKey) {
                clear();
                return true; // Don't set scripts to triggered if we just cleared.
            }

            const auto enableScripts = [alwaysBlockHotkeyKeys = this->alwaysBlockHotkeyKeys, pressedKey, &triggered, &enable_state_changed](std::vector<Script>& scripts) {
                for (auto& script : scripts) {
            ...
            if (enable_state_changed) refreshDisabledKeys();
...
void SpeedrunScriptingTools::Initialize(ImGuiContext* ctx, const ImGuiAllocFns allocator_fns, HMODULE toolbox_dll)
{
    ...
    terminating = false;
    pendingDisplayDialogDecodes = 0;
    activeScriptingTools = this;
    ...
    GW::UI::RegisterUIMessageCallback(&FinishSkillCast_Entry, kSkillCooldownStart, [this](GW::HookStatus*, GW::UI::UIMessage, void* wparam, void*) {
    ...
    GW::UI::RegisterUIMessageCallback(&GotoTargetDialog_Entry, GW::UI::UIMessage::kDialogBody, [](GW::HookStatus*, GW::UI::UIMessage, void* wParam, void*) {
        ...
        const auto node = gotoTargetDialogPoppedUp.extract(packet->agent_id);
        if (node) {
            for (auto dialogHasPoppedUp : node.mapped()) {
...
void SpeedrunScriptingTools::SignalTerminate()
{
    ...
    activeScriptingTools = nullptr;
    ToolboxUIPlugin::SignalTerminate();
}

bool SpeedrunScriptingTools::CanTerminate()
{
    return pendingDisplayDialogDecodes.load() == 0;
}

void SpeedrunScriptingTools::OnDisplayDialogDecoded(void* context, const wchar_t* decoded)
{
    if (const auto instance = static_cast<SpeedrunScriptingTools*>(context)) {
        instance->CompleteDisplayDialogDecode(decoded);
    }
}
...
{
    clear();
    gotoTargetDialogPoppedUp.clear();
    repopMinipets.clear();
    ScriptVariableManager::getInstance().reset();
