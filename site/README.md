...
(`/linux` → `/docs/linux/`), generated by
[`src/pages/[legacySlug].astro`](src/pages/[legacySlug].astro) from the same
content collection — no per-page bookkeeping needed. It redirects client-side
(with a `<meta http-equiv="refresh">` fallback) so that the anchor in a deep
link like `/minimap#agents` survives the hop; Astro's built-in `redirects`
config emits a bare meta refresh, which drops the fragment.

Note that the site only deploys from `master` (see
`.github/workflows/site-deploy.yml`), so doc changes merged to `dev` stay
invisible until `master` is updated.
