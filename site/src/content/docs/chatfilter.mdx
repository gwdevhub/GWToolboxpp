...
- A line wrapped in slashes is always a regex, e.g. `/wts.*ecto/`, whichever box it's in.
...
Block specific players' messages outright. One name per line, matched in full and not case-sensitive; a line wrapped in slashes is a regex instead, e.g. `/^Bob The/`.
...
For more complex needs, tick **Use regular expressions** and treat each line of the custom-filter list as a regex pattern. Lines in that list stay case-sensitive unless wrapped in slashes with the `i` flag, e.g. `/wts.*ecto/i`.

The same slash syntax works everywhere Toolbox takes a filter you typed — chat filters, [trade and party-search alerts](./trade/) and [loot beacon name rules](./loot_beacons/).
