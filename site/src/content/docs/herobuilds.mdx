...
<figure>
<img src="/docs-images/kwop9ub.webp"/>
...
If you have mercenary heroes, the hero selector dropdown will automatically display their **in-game names** and **profession icons** (e.g., "Kishwa" with an Assassin icon) when you're in an outpost. Outside of outposts, mercenaries fall back to default labels ("Mercenary 1", etc.) since profession data isn't available.

No configuration is required — mercenary names and professions are read directly from the game.
...
- **Sort heroes by profession**: Groups heroes by profession in the hero selector dropdown. When disabled, heroes are sorted alphabetically. Off by default.
