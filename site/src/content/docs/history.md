...
* [New] World map: a "Show portals" tickbox in the world map controls draws the travel portals of every map you have visited on the current continent, with hover details for the portal it links to. The portal data is now also saved and reloaded correctly between sessions — the file it was written to could not be read back.
...
* [Fix] Salvage: items the game has no salvage formula for — such as Stalker's Ration and other bundle-style items — are no longer treated as salvagable, so "Salvage All" stops queueing them up. Previously only one hard-coded formula was excluded, which let these items through: the kit would pause on them and then report a salvage failure for that slot.
