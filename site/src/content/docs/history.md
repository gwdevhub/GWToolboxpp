...
## Version 8.34
* [Fix] Objective Timer: collapsed past runs are visible again in the run history window.
* [Perf] Loot Beacons now update when item agents appear or disappear instead of repeatedly scanning every agent. Custom name rules are applied once the item's name has decoded.
...
* [New] World map: elite skill capture icons can now have profession-coloured borders, using the profession colours configured in Minimap settings. The option is enabled by default, and the icon tooltip also shows the profession icon.
...
* [Fix] Salvage: items the game has no salvage formula for — such as Stalker's Ration and other bundle-style items — are no longer treated as salvageable, so "Salvage All" stops queueing them up. Previously only one hard-coded formula was excluded, which let these items through: the kit would pause on them and then report a salvage failure for that slot.
...
* [Minor] The [Flag Hero hotkey](/docs/hotkeys/#flag-hero) is properly documented: its Degree/Distance/Hero fields, the fact that the angle is measured from the line between you and your target when you have one (and from your facing direction otherwise), that pressing the hotkey again unflags, and how to build hero formations out of a handful of them.
