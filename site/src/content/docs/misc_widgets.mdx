...
GWToolbox++ includes several small but useful widgets that enhance your gameplay experience. Each lives in its own settings section — see the heading on each entry below for the exact path.

## Latency Widget

The Latency Widget displays your current ping to the server. It helps you monitor your connection quality in real-time.

<ToolboxPath steps={['Settings', 'Latency']} />
...
<ToolboxPath steps={['Settings', 'Clock']} />
...
<ToolboxPath steps={['Settings', 'Vanquish']} />
...
For an overlay drawn on top of the in-game mission map, see the [Vanquish Overlay](/docs/vanquish_overlay/) doc.

## Server Info Widget

The Server Info Widget displays information about the game server you're connected to.

<ToolboxPath steps={['Settings', 'Server Info']} />
...
<ToolboxPath steps={['Settings', 'Alcohol']} />
...
The World Map Widget adds extra functionality to the in-game world map — see the dedicated [World Map](/docs/world_map/) doc for the full feature list.

<ToolboxPath steps={['Settings', 'World Map']} />
...
Open from <ToolboxPath kind="window" steps={['Toolbox', 'Notepad']} /> or with <ToolboxPath kind="command" steps={['/show notepad']} />.
