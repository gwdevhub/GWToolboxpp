...
<ToolboxPath steps={["Settings", "Game Settings"]} /> contains **Agent Colors**, **Agent Sizes**, and **Agent Appearance**. These settings control [Minimap](/docs/minimap/) markers and in-game name tags even when the Minimap widget is disabled. Appearance rules are saved in `configs/default/modules/Game Settings.json`; existing rules in `Minimap.json`, `AgentColors.json` and `AgentColors.ini`, and old Game Settings/Friend List nametag colours are imported automatically.

- **Agent Colors** — colours of EoE, QZ, Winnowing, Frozen Soil and Symbiosis auras, plus agent shading and damaged-agent modifiers. **Restore Defaults** resets the effect colours and seeded appearance rules. The **Agent Border thickness** and **Target Border thickness** sliders below the rule list control marker outlines.
- **Agent Sizes** — fallback size and shape for agents without a matching size or shape rule. Player, boss, item, gadget and minion defaults are seeded in the rules below.
- **Agent Appearance** — an ordered, filterable list of rules. The first matching enabled field wins; other fields fall through to later rules or the fallback. **Add Appearance Rule** creates a row. Seeded defaults for allies, enemies, players, items, gadgets, chests, marked targets and profession colours can be edited, moved or disabled, but not deleted.
- Each rule can match an **Agent type** (`Item`, `Gadget`, `NPC`, `Player` or `Any`), an optional type-specific **Identifier** (item model ID, gadget ID or NPC model ID), **Allegiance**, **Map ID**, and **Match name**. Check **Match identifier** for an exact ID match, including ID `0`; leave it unchecked to match any ID of that type. Name matching uses the same case-insensitive substring or `/pattern/flags` regex syntax as Loot Beacons. During migration, old allegiance entries become NPC rules and old model-ID entries become adjacent NPC and gadget rules, preserving their order.
- Optional state filters include **Target state**, **Dead state**, **Quest state**, **Combat**, **Weapon**, **Player relation**, **Gadget state**, **Profession**, **Boss state** and **Outposts only**. Rules can override minimap **Color**, **Size**, **Shape** and target border colour independently of in-game **Text color**. Set a colour's alpha to `0`, size to `0`, or shape to **Inherit** to fall through to the next matching rule or default. Older per-field enable checkboxes are converted to these values when settings are migrated.
...
* **Agent and name tag colours** are configured together under [Agent Appearance](#agent-appearance). Set a row's **Text color** for the in-game name tag; set its alpha to `0` to inherit the next rule or Guild Wars' colour.
