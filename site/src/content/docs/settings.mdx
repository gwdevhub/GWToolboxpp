...
- **Agent Colors** — colours of EoE, QZ, Winnowing, Frozen Soil and Symbiosis auras, plus agent shading and damaged-agent modifiers. **Restore Defaults** resets effect and fallback colours, without changing appearance rules. The **Agent Border thickness** and **Target Border thickness** sliders below the rule list control marker outlines.
- **Agent Sizes** — fallback size and shape for agents without a matching size or shape rule. **Restore Defaults** resets fallback sizes, not appearance rules.
- **Agent Appearance** — an ordered, filterable list of rules. The first matching enabled field wins; other fields fall through to later rules or the fallback. **Add Appearance Rule** creates a row. Initial rules for allies, enemies, players, items, gadgets, chests, marked targets and profession colours are ordinary rules: edit, move, disable or delete them like any other.
