...
- Each rule can match an **Agent type** (`Item`, `Gadget`, `NPC`, `Player` or `Any`), an optional type-specific **Identifier** (item model ID, gadget ID or NPC model ID), **Allegiance**, **Map ID**, and **Match name**. Name matching uses the same case-insensitive substring or `/pattern/flags` regex syntax as Loot Beacons.
