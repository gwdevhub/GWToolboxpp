...
Open from <ToolboxPath kind="window" steps={["Toolbox", "Trade"]} /> or <ToolboxPath kind="command" steps={["/show trade"]} />. Per-window options live under <ToolboxPath steps={["Settings", "Trade"]} />.

<img src="/docs-images/trade_window.webp" alt="Trade window showing a live feed of Kamadan trade chat with timestamps, sender names and messages" />
...
A line wrapped in slashes is treated as a regular expression instead of a plain keyword, e.g. `/(wtb|wts).*ecto/`. Flags go after the closing slash — matching is case-insensitive unless you add `I`. The same syntax works for the party search alerts and for [loot beacon name rules](./loot_beacons/).
