...
<figure>
<img src="/docs-images/28233518-40a6f0d4-68ac-11e7-96c5-5c4fab47a107.webp"/>
<figcaption>Travel Window</figcaption>
</figure>

Open from <ToolboxPath kind="window" steps={["Toolbox", "Travel"]} /> or <ToolboxPath kind="command" steps={["/show travel"]} />. Per-window options live under <ToolboxPath steps={["Settings", "Travel"]} />.
...
## User Travel Destinations
The quick-access buttons in the Travel window are fully configurable. To manage them, go to <ToolboxPath steps={["Settings", "Travel"]} /> and scroll to the **User Travel Destinations** section:
- **Add**: Click "Add Destination" to append a new button. Select the outpost, district, and optional district number for each entry.
- **Remove**: Click the trash button on any row to delete that destination.
- **Reset**: "Reset to Defaults" restores the original set of destinations.

Note: reordering destinations is not currently supported in the UI.
...
## Settings
Open via <ToolboxPath steps={["Settings", "Travel"]} />.
...
- **Automatically retry if the district is full**: Keep trying to enter a full district. Use `/tp stop` to cancel.
- **Use English map names**: When unchecked, the `/tp` command uses localized map names matching your current language.
- **Show Zaishen quest buttons**: Toggle the Zaishen Bounty, Mission, Vanquish, and Combat travel buttons in the Travel window.
- **User Travel Destinations**: Add or remove the quick-travel buttons shown in the Travel window (see [above](#user-travel-destinations)).
- **Outpost Aliases**: Add, edit, or remove shorthand aliases used with the `/tp` command.
