#!/usr/bin/env python3
"""
Scrapes Guild Wars wiki to generate C++ zaishen coin reward data.

Usage:
    python3 tools/generate_zaishen_coins.py

Outputs a C++ snippet ready to paste into DailyQuestsWindow.cpp.
Requires network access to wiki.guildwars.com.
"""
...
WIKI_BASE = "https://wiki.guildwars.com/wiki/Game_link:Quest_{}"
HEADERS = {"User-Agent": "GWToolboxpp/generate_zaishen_coins"}

# All zaishen quest IDs from GWCA/Constants/QuestIDs.h, keyed by enum name
ZAISHEN_QUEST_IDS: dict[str, int] = {
    ...
    # Zaishen Mission - Extra (non-sequential IDs)
...
def fetch_page(quest_id: int) -> str | None:
    url = WIKI_BASE.format(quest_id)
    req = urllib.request.Request(url, headers=HEADERS)
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            return resp.read().decode("utf-8", errors="replace")
    except Exception as e:
        print(f"  Warning: failed to fetch quest {quest_id}: {e}")
        return None


def parse_coin_rewards(html: str) -> tuple[int, int]:
    """
    Extract (nm_coins, hm_coins) from a Game_link:Quest wiki page.

    The GW wiki quest infobox lists reward items in a table. Copper Zaishen
    Coins appear as rows in that table with a quantity. NM rewards precede HM
    rewards in document order. Returns (0, 0) if nothing is found.
    """
    # Normalise whitespace so our regex patterns are simpler
    text = re.sub(r"\s+", " ", html)

    # Find every mention of a Copper Zaishen Coin quantity on the page.
    # The wiki renders quantities with patterns like:
    #   >1<  ...Copper Zaishen Coin
    #   (1)  ...Copper Zaishen Coin
    #   Copper Zaishen Coin (1)
    all_quantities: list[int] = []

    # Broad sweep: grab every digit within 200 chars before or after the coin name
    for m in re.finditer(r"Copper Zaishen Coin", text, re.IGNORECASE):
        region = text[max(0, m.start() - 200): m.end() + 200]
        nums = re.findall(r"\b(\d+)\b", region)
        for n in nums:
            v = int(n)
            if 1 <= v <= 100:  # sanity range
                all_quantities.append(v)

    if not all_quantities:
        return (0, 0)

    if len(all_quantities) == 1:
        # Only one value found — assume it applies to both modes
        return (all_quantities[0], all_quantities[0])

    # The wiki page lists NM rewards before HM rewards; take the first two
    # unique values as (nm, hm). If they're the same, HM is simply double.
    nm = all_quantities[0]
    hm = all_quantities[1] if all_quantities[1] != nm else nm * 2
    return (nm, hm)


def main() -> None:
    results: dict[str, tuple[int, int]] = OrderedDict()
    total = len(ZAISHEN_QUEST_IDS)
    for i, (name, qid) in enumerate(ZAISHEN_QUEST_IDS.items(), 1):
        print(f"[{i}/{total}] Fetching quest {qid} ({name})...")
        html = fetch_page(qid)
        if html:
            nm, hm = parse_coin_rewards(html)
        else:
            nm, hm = 0, 0
        results[name] = (nm, hm)
        if i < total:
    ...
    # Emit C++ code
    print()
    print("// Copper Zaishen Coin rewards by quest ID.")
    print("// Generated from Guild Wars wiki via tools/generate_zaishen_coins.py")
    print("// Keyed by GW::Constants::QuestID; values are {nm_coins, hm_coins}.")
    print("const std::unordered_map<GW::Constants::QuestID, DailyQuests::ZaishenCoinReward> zaishen_coin_rewards = {")

    current_prefix = ""
    for name, (nm, hm) in results.items():
        prefix = name.split("_")[0]  # ZaishenMission / ZaishenBounty / ZaishenVanquish
        if prefix != current_prefix:
            if current_prefix:
                print()
            print(f"    // {prefix}")
            current_prefix = prefix
        print(f"    {{QuestID::{name}, {{{nm}, {hm}}}}},")
